...
// Valid image path checker 
bool IsPathExist(const std::string &s){
  struct stat buffer;
  return (stat (s.c_str(), &buffer) == 0);
}

// default return varibales
struct vect
    {            
	bool tf = false;
	double max_scores = -(std::numeric_limits<double>::max()) - 1;
	//vector<int> row;
	std::vector<std::vector<int>> vect1 {{0}};	
    }; 
...
int main(int argc, char ** argv) {
    ...
    //try{
	// test for the same image, with only one face
        //std::string img_path = "/home/varunsakunia/dlib_test/id01.jpg";    
	//std::string Img_path = img_path;
//	if (IsPathExist(img_path)){
    	// checking
//    	cout << "Image Loading is successful" << endl;
//  	}else{ 
//		throw (img_path);
//	}
//    }

//    catch (std::string img_path)
 //   {
    // Error return values	    
    	//struct retVals {
	//bool tf = false;		
    	//double max_scores = -(std::numeric_limits<double>::max()) - 1;
	//vector<vector<int>> vect( 1 , vector<int> (1, 0));
 //	 };
 //   cout << "Error in loading file" << endl;
 //   return -1;
    //return retVals {tf, max_scores, vect};

//    }

    //checking image path
    if (IsPathExist(img_path) == false)
    {
 	cout << "Error in loading file" << endl;
	vect v;
	cout << std::boolalpha << v.tf << " ";
	cout << v.max_scores << " "; 
	
	//printing vector
	for (int i = 0; i < v.vect1.size(); i++)
    	    {
                for (int j = 0; j < v.vect1[i].size(); j++)
        	{
                    cout << v.vect1[i][j] << " ";
                }
                cout << endl;
    	    }
        //return {p.tf, p.max_scores, p.M};
	//cout << std::boolalpha << v.tf << ", " <<v.max_scores;
	return -1;
    }
    ...
    cv::Mat img_final_rotation = Image.clone();
    std::vector<double> final_dets;
    double max_scores = -(std::numeric_limits<double>::max()) - 1;
